...
class Trainer(BaseTrainer):
    ...
    def _train_epoch_blip(self, epoch):
        ...
        for batch_idx, (images, captions, cls_labels, clip_memory) in enumerate(self.train_dataloader):
            ...
            loss_lm, loss_cls, loss_cpc = self.model(images, captions, cls_labels, clip_memory, self.criterion_cls, self.base_probs)
            loss = loss_lm + self.args.cls_weight*loss_cls + 0.1 * loss_cpc
            if batch_idx%10 == 0:
                print("{}/{} loss: {}, loss_lm: {}, loss_cls: {}, loss_cpc: {}".format(batch_idx, len(self.train_dataloader), loss.item(), loss_lm.item(), self.args.cls_weight*loss_cls.item(), 0.1 * loss_cpc.item()))
