...
class BLIP_Decoder(nn.Module):
    def __init__(self,                 
        ...
        self.text_decoder = BertLMHeadModel.from_pretrained('/picassox/kh-mvv-data/digital-human/workspace/code/fzw/utils/metric/checkpoints/bert-base-uncased',config=decoder_config)
    ...
    def forward(self, image, caption, cls_labels, clip_memory, criterion_cls, base_probs):
        ...
        # print(image_embeds.shape, avg_embeds.shape)

        ##########################
        # NxKxC -> KxNxC
        query_embed = self.vision_proj(avg_embeds)

        diff_prompt_l2 = torch.norm(query_embed - clip_memory.mean(dim=1), dim=-1, keepdim=True)
        diff_prompt_cosine = torch.nn.functional.cosine_similarity(
            query_embed, 
            clip_memory.mean(dim=1), 
            dim=-1, 
            eps=1e-8
        ).unsqueeze(-1)
        diff_prompt_dot = torch.sum(query_embed * clip_memory.mean(dim=1), dim=-1, keepdim=True)
        
        diff_prompt_features = torch.cat((diff_prompt_l2, diff_prompt_cosine, diff_prompt_dot), dim=-1)
        diff_prompt = self.diff_mlp(diff_prompt_features)

        clip_memory = torch.permute(clip_memory, (1, 0, 2))
        hs = self.memory(clip_memory, None, query_embed.unsqueeze(0), None)

        diff_hs = self.memory(clip_memory, None, diff_prompt.unsqueeze(0), None)
        ...
        image_embeds_with_diff = image_embeds + diff_hs.squeeze(0).repeat(1, image_embeds.size(1), 4)
        ...
        decoder_output = self.text_decoder(text.input_ids, 
                                           ...
                                           encoder_hidden_states = image_embeds_with_diff,
        ...
        positive_similarity = torch.nn.functional.cosine_similarity(query_embed, hs.unsqueeze(1), dim=-1)
        negative_similarity = torch.nn.functional.cosine_similarity(query_embed, diff_hs.unsqueeze(1), dim=-1)
        ...
        return loss_lm, loss_cls, loss_cpc
    ...
    def generate(self, image, clip_memory, sample=False, num_beams=3, max_length=100, min_length=10, top_p=0.9, repetition_penalty=1.0):
        ...
        if not sample:
            image_embeds = image_embeds + diff_hs.squeeze(0).repeat(1, image_embeds.size(1), 4)
